...
void GeneralMatrixMul(int* A, int* B, int* C) {
    ...
    for (int i = 0; i < (M + TILE - 1) / TILE; i++) {
        ...
        if (i * TILE + ty < M && bx * TILE + tx < K) {
            Bds[ty][tx] = B[(i * TILE + ty) * K + bx * TILE + tx];
        } else {
            Bds[ty][tx] = 0;
        }
        __syncthreads(); // read after write
        for (int j = 0; j < TILE; j++) {
            res += Ads[ty][j] * Bds[j][tx];
        }
        __syncthreads(); // write after read
    }
    if (by * TILE + ty < N && bx * TILE + tx < K) {
        C[(by * TILE + ty) * K + bx * TILE + tx] = res;
    }
}
...
int main() {
    ...
    dim3 grid((K + TILE - 1) / TILE, (N + TILE - 1) / TILE); // 注意这里，只跟ouput的维度相关
