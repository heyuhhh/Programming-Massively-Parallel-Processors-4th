...
int co_rank(int k, volatile float* A, int n, volatile float* B, int m) {
    ...
    while (l < r) {
        mid = (l + r) >> 1;
        if (k - mid - 1 >= 0 && A[mid] < B[k - mid - 1]) {
            l = mid + 1;
        } else if (k - mid >= 0 && mid > 0 && B[k - mid] < A[mid - 1]) {
            r = mid;
        } else {
            break;
        }
...
int co_rank_cir(int k, float* A, int n, float* B, int m, int A_start, int B_start) {
    ...
    while (l < r) {
        ...
        if (k - mid - 1 >= 0 && A[A_ind] < B[(B_ind - 1 + TILE_SIZE) % TILE_SIZE]) {
            l = mid + 1;
        } else if (k - mid >= 0 && mid > 0 && B[B_ind] < A[(A_ind - 1 + TILE_SIZE) % TILE_SIZE]) {
            r = mid;
        } else {
            break;
        }
...
void circularMergeSort(float* A, float* B, float* C) {
    ...
    int C_cur = blockIdx.x * TILE_SIZE * COARSE_FACTOR;
    int C_nxt = min((blockIdx.x + 1) * TILE_SIZE * COARSE_FACTOR, K);
    // 找到这个块的下标范围
    if (threadIdx.x == 0) {
...
int main() {
    ...
    dim3 gridDim((K + TILE_SIZE * COARSE_FACTOR - 1) / (TILE_SIZE * COARSE_FACTOR));
    dim3 blockDim(128);
